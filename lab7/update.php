<?php
    ...
    if (!empty($_POST)) {
        ...
        if ($valid) {
            ...
            $sql = 'update cars set model = ?, manufacturer = ?, power = ?, color = ?, age = ?, price = ? where id=?';
            $q = $pdo->prepare($sql);
            $q->execute(array($model, $manufacturer, $power, $color, $age, $price, $id));
            Database::disconnect();
            header('Location: index.php');
        }
    } else {
...
<html>
    <head>
        <meta charset="utf-8">
        <title>Update Item <?php echo $id; ?></title>
